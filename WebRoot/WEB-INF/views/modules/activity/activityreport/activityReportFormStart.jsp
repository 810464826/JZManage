	...
	$(document).ready( function() {
		...
		//点击提交和保存按钮分别进行验证
			$("#btnSave,#btnSubmit").click(function(){
				...
				for(var i in selectedLists){
					if(i==0){
						recordId=selectedLists[i].recordId;
					}else{
						recordId=recordId+","+selectedLists[i].recordId;
					}
						...
						　　},
						   error:function(){
							...
							/* <c:forEach items='${page}' var="signGoods">
								...
								waybill.goodsNumber = '${signGoods.outNo}';
								waybill.recordId = '${signGoods.recordId}';
								waybill.dealerName = '${signGoods.sellerName}';
								waybill.varieties = '${signGoods.productName}';
								waybill.totalBox = '${signGoods.boxCode}';
								waybill.createTime = getTaskTime('${signGoods.outDate}');
								waybill.poductsize='${signGoods.spec}';
						   ...
						   // });
							...
							$("#proNameSearch").on("click",function(){
								var productName=$("#proName").val();
								//alert(productName);
								if(productName!=""){
									$.each($(".proName"),function(){
										if($(this).html().indexOf(productName)>-1){
											$(this).parent().show();
										}else{
											$(this).parent().hide();
										}
	...
	function CreateTableWayBill(rowCount, content) {
		...
		for (var i = 0; i < rowCount; i++) {
			...
			var td1 = $('<td>' + content[i].goodsNumber
					+ '</td>');
			var td2 = $('<td>' + content[i].recordId
					+ '</td>');
			var td3 = $('<td>' + content[i].varieties
					+ '</td>');
			var td4 = $('<td>' + content[i].totalBox
					+ '</td>');
			var td5 =$('<td>' + content[i].poductsize
					+ '</td>');
			var td6 = $('<td>'
...
var selectedLists = []; //在运单中心选中的数据：复选框已选择的数据集合
var selectedListsCopy = []; //复制billLists的数据，与其一样
...
function isSelected() {
	...
	for ( var i in selectedListsCopy) {
		$(".checkBox input[type='checkbox']").each(function() {
			if (selectedListsCopy[i].recordId == $(this).val()) {
...
function isSelectedCheckbox(currentObj) {
	...
	if (isCurrentSelected == true) {
		var obj = {};
		obj.recordId = $(currentObj).val(); //发货记录ID
		obj.goodsNumber = $(currentObj).parent().siblings().eq(0).html(); //发货单号
		obj.dealerName = $(currentObj).parent().siblings().eq(2).html();  //渠道商名称
		obj.varieties = $(currentObj).parent().siblings().eq(3).html(); 	//品种名称
		obj.totalBox = $(currentObj).parent().siblings().eq(4).html();		//总箱数
		obj.createTime = $(currentObj).parent().siblings().eq(5).html();	//发货时间
		obj.poductsize = $(currentObj).parent().siblings().eq(6).html();	//规格
		selectedListsCopy.push(obj);
	} else {
		for ( var i in selectedListsCopy) {
			if (selectedListsCopy[i].recordId == $(currentObj).val()) {
				selectedListsCopy.splice(i, 1);
				return;
			}
		}
	}
...
$(function(){
	$("#confirmSelecte").on("click", function() {
		$('#myModal').hide();
		$('.reveal-modal-bg').hide();
		selectedLists = deepCopy(selectedListsCopy);
		if(selectedLists.length>0) {
			$(".award-button").removeAttr("disabled");
		}else{
			$(".award-button").attr("disabled","disabled");
		}
		CreateTableWayBill(selectedLists.length,selectedLists);
	});
})
...
$(function() {
	...
	$("#proNameSearch").click(function() {
		...
		$.ajax({
			...
			type: "GET",
			dataType: "JSON",
			url: "${ctx}/activityreport/activityReport/loadSellerInfo",
			success: function(data) {
				if(data != false) {
					checkLoad(data);
				} else {
...
function checkLoad(data) {
	...
	for(var i = 0; i < data.outList.length; i++) {
		...
		adminObj.find(".sellerName").text(adminArray.recsellerName);
		adminObj.find(".productName").text(adminArray.productName);
		adminObj.find(".boxCode").text(adminArray.boxCount);
		adminObj.find(".outDate").text(adminArray.outTime);
		adminObj.find(".spec").text(adminArray.spec);
		adminObj.find(".checkBox input[type='checkbox']").val(adminArray.recordId);
	...
	<form:form id="inputForm" modelAttribute="activityReport"
		...
		<div id="myModal" class="reveal-modal">
			<div>
				...
				<div class="proSearch"> 
					品种名称：<input type="text" id="proName" />
					<input type="button" value="查询" id="proNameSearch" />
			...
			<div style="height: 460px; overflow-y: auto; margin: 16px auto">
				<table id="contentTable"
					...
					<tbody>
						<%-- <c:forEach items="${page}" var="sellerOutInfo">
							<tr>
								<td>${sellerOutInfo.outNo}</td>
								<td>${sellerOutInfo.recordId}</td> 
								<td>${sellerOutInfo.sellerName}</td>
								<td class="proName">${sellerOutInfo.productName}</td>
								<td>${sellerOutInfo.boxCode}</td>
								<td><fmt:formatDate value="${sellerOutInfo.outDate}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td>${sellerOutInfo.spec}</td>
								<td><input type="checkbox" value="${sellerOutInfo.outNo}"
									name="checkbox" /></td>
							</tr>
						</c:forEach> --%>
					</tbody>
			...
			<div style="display: none;">
				<table>
					<tr class="admin-obj">
						...
						<td class="outDate"></td>
						<td class="spec"></td>
						<td class="checkBox"><input type="checkbox" value="" name="checkbox" onClick="isSelectedCheckbox(this)" /></td>
					</tr>
				</table>
			</div>
			<!-- 分页结束 -->
