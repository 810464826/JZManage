...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var roles=$("#roles option").eq(parseInt(officetype)-1).val();
			$("#roles").change(function() {
				roles = $("#roles option:selected").val();
			});
			
			
			var optionVal=$("#roles option").eq(parseInt(officetype)-1).text();
			$("#s2id_roles .select2-chosen").html(optionVal);
			$("#roles").find("option[value = '"+officetype+"']").attr("selected","selected");
			$("#btnSubmit").on("click",function(){
				var actionUrl="${ctx}/sys/user/save?roles="+roles;
...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<input type="hidden" id="officetype" value="${user.userType}"/>
		</div>
