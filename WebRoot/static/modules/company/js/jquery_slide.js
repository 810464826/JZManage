   ...
   function pos_dex(N){ 
	...
	//	alert(z);
		...
		for(i=1;i<=half;i++){
			 ...
			 li.eq(next).animate({"left":"1230px","width":w2,"height":h2,"top":top2},a_speed);
			 // li.eq(next).css("z-index",z);
			  //left 
			  var pre=N-i;
			  if(pre==-1){
			 ...
			 // li.eq(pre).css("z-index",z);
			  li.eq(pre).animate({"left":"-850px","width":w2,"height":h2,"top":top2},a_speed);
			} 
			//mid
	       if(length%2==0){
			  li.eq(next+1).attr("class","mid");
			 li.eq(next+1).css("z-index",z-2);
			  li.eq(next+1).animate({"left":"1230px","width":w2,"height":h2,"top":top2},a_speed);
			  }
